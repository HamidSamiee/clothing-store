...
const handler: Handler = async () => {
  try {
    // دریافت تمام محصولات بدون فیلتر
    const queryStr = 'SELECT * FROM products';
    const productsResult = await query<Product>(queryStr);
    ...
    return {
      ...
      body: JSON.stringify(productsResult.rows)
    };
  } catch (err) {
    console.error('Get all products error:', err);
    return {
      statusCode: 500,
      body: JSON.stringify({ message: 'خطای سرور در دریافت محصولات' })
    };
  }
};
